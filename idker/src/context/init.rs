...
use x86_64::{VirtAddr, structures::paging::{FrameAllocator, Mapper, OffsetPageTable, Page, PageSize, PageTableFlags, Size4KiB}};

use crate::{allocator::get_frame_allocator, arch::paging::physical_memory_offset, include_bytes_align_as};

// TODO: proper init directory management, we won't use only initproc
static INIT_DATA: &'static [u8] = include_bytes_align_as!(u64, r"../../../initproc/target/x86_64-smoke/debug/initproc");
...
pub fn mount(table: &mut OffsetPageTable) -> Elf<'static> {
    let mut allocator = get_frame_allocator();
    let offset = physical_memory_offset();

    let elf = Elf::new(INIT_DATA).expect("Invalid init ELF format");
    let headers = elf.programs()
