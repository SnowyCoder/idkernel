...
use x86_64::structures::paging::{FrameAllocator, Mapper, Page, PageTableFlags, PhysFrame, Size4KiB, page};
use core::intrinsics::{atomic_load, atomic_store};
use core::sync::atomic::{AtomicBool, Ordering};
use crate::arch::x86_64::paging::{get_page_table, physical_memory_offset};
use crate::arch::x86_64::apic::{LOCAL_APIC, LocalApic};
...
pub fn init_ap_processor(p: &Processor, frame_allocator: &mut impl FrameAllocator<Size4KiB>) {
    ...
    unsafe { page_table.identity_map(first_frame, flags, frame_allocator).unwrap().flush(); }
    for _ in 1..STACK_FRAME_COUNT {
        let frame = frame_allocator.allocate_frame().expect("Unable to allocate frame");
        unsafe { page_table.identity_map(frame, flags, frame_allocator).unwrap().flush(); }
    }
...
pub fn init(frame_allocator: &mut impl FrameAllocator<Size4KiB>, proc_info: &ProcessorInfo) {
    println!("MULTI_CORE INIT!");

    println!("Writing trampoline...");
    let frame = frame_allocator.allocate_frame().expect("Error allocating backup frame");
    Trampoline::write(frame, frame_allocator);

    proc_info.application_processors.iter()
        .filter(|p| p.state != ProcessorState::Disabled)
        .for_each(|p| init_ap_processor(p, frame_allocator));
...
impl Trampoline {
    ...
    unsafe fn unwrite(frame: PhysFrame) {
        let from  = VirtAddr::new(frame.start_address().as_u64());
        let dest = VirtAddr::new(TRAMPOLINE_ADDR + physical_memory_offset().as_u64());
        for i in 0..TRAMPOLINE_DATA.len() {
            unsafe {
