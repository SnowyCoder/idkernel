...
use x86_64::{VirtAddr, registers::model_specific::FsBase, structures::paging::{FrameAllocator, Mapper, Page, PageTableFlags, Size4KiB}};

use crate::arch::x86_64::consts::{KERNEL_THREAD_DATA_LENGTH, KERNEL_THREAD_DATA_START};

use super::{BootInfoFrameAllocator, get_page_table};
...
fn allocate_map_range(addr: VirtAddr, size: u64, flags: PageTableFlags, frame_allocator: &mut BootInfoFrameAllocator) {
    ...
    for page in Page::range_inclusive(first_page, last_page) {
        let frame = frame_allocator.allocate_frame().expect("Error: out of memory");
        unsafe {
...
pub unsafe fn setup_thread_data(cpu: u64, frame_allocator: &mut BootInfoFrameAllocator) {
    let start_addr = VirtAddr::new(KERNEL_THREAD_DATA_START + (cpu as u64) * KERNEL_THREAD_DATA_LENGTH);
    allocate_map_range(start_addr, KERNEL_THREAD_DATA_LENGTH, PageTableFlags::PRESENT | PageTableFlags::WRITABLE, frame_allocator);
    ...
    if tdata_len + tbss_len > KERNEL_THREAD_DATA_LENGTH as usize {
        panic!("Cannot fit tdata + tbss into the allocated length ({:x} + {:x} > {:x}), allocate more", tdata_len, tbss_len, KERNEL_THREAD_DATA_LENGTH);
    }
    ...
    ptr::write_bytes::<u8>(start_addr.as_mut_ptr::<u8>().offset(tdata_len as isize), 0, tbss_len);
