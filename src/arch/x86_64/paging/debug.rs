...
use x86_64::structures::paging::{PageSize, PageTable, PageTableFlags, Size1GiB, Size2MiB, Size4KiB};
...
pub fn explore_page_ranges() -> impl Iterator<Item=(usize, usize, PageTableFlags)> {
    let my_genm = GenIter::from(static || {unsafe {
        ...
        for (i4, x4) in table.iter().enumerate() {
            ...
            let start4 = i4 * 512 * Size1GiB::SIZE as usize;
            for (i3, x3) in (&*(offset + x4.addr().as_u64()).as_ptr() as &PageTable).iter().enumerate() {
                ...
                for (i2, x2) in (&*(offset + x3.addr().as_u64()).as_ptr() as &PageTable).iter().enumerate() {
                    ...
                    for (i1, x1) in (&*(offset + x2.addr().as_u64()).as_ptr() as &PageTable).iter().enumerate() {
    ...
    let removed_fields = PageTableFlags::ACCESSED | PageTableFlags::DIRTY |  PageTableFlags::HUGE_PAGE;
    my_genm.map(move |(f, t, flags)| (f, t, flags - removed_fields))
